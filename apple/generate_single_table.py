...
sleep_data.head()
#%%
sleep_data_by_day = sleep_data.groupby('creationDateTime').agg(total_time_asleep=('time_asleep', 'sum'),
    bed_time=('startDateTime', 'min'), 
    awake_time=('endDateTime', 'max'), 
    sleep_counts=('creationDateTime','count'))
# %%
sleep_data_by_day['time_in_bed'] = sleep_data_by_day['awake_time'] - sleep_data_by_day['bed_time']
sleep_data_by_day['restless_time'] = sleep_data_by_day['time_in_bed'] - sleep_data_by_day['total_time_asleep']
...
# %% plot
sleep_data_by_day['time_in_bed'] = (sleep_data_by_day['time_in_bed'].dt.total_seconds()/60)
sleep_data_by_day['total_time_asleep'] = (sleep_data_by_day['total_time_asleep'].dt.total_seconds()/60)
sleep_data_by_day[['time_in_bed','total_time_asleep']].plot(use_index=True)
plt.show()
